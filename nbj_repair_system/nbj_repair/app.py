    ...
    if conn:
        try:
            ...
            cols = [
                ...
                "ALTER TABLE repairs ADD COLUMN technician_notes TEXT AFTER issue_description", # Added for tracking display
...
def admin_dashboard():
    ...
    try:
        ...
        stats = {
            ...
            'repairs': rows # Added to ensure dashboard lists work
        }
...
def track():
    ...
    if request.method == 'POST':
        ...
        try:
            conn = get_db_connection()
            cursor = conn.cursor(dictionary=True)
            # Search for the exact tracking number
            ...
            if not repair:
                error = f"No repair record found for ID: {tracking_number}"
        except Exception as e:
