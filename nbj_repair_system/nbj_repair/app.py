...
def setup_database():
    ...
    if conn:
        try:
            ...
            # This ensures EVERY column your dashboard and track page needs exists
            cols = [
                ...
                "ALTER TABLE repairs ADD COLUMN technician_notes TEXT",
...
# --- ADMIN DASHBOARD (Likely where the 500 error was) ---
...
def admin_dashboard():
    ...
    try:
        ...
        # Using .get() prevents crashing if a status is missing
        stats = {
            'total': len(rows),
            'ongoing': sum(1 for r in rows if str(r.get('status')) == 'Received'),
            'waiting': sum(1 for r in rows if str(r.get('status')) == 'Awaiting Parts' or str(r.get('status')) == 'Waiting for Parts'),
            'completed': sum(1 for r in rows if str(r.get('status')) == 'Completed'),
            'recent': rows[:5],
            'repairs': rows, # Crucial: Your HTML likely loops through 'repairs'
...
def track():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            cursor.close()
            conn.close()
            if not repair: error = f"ID '{tracking_number}' not found."
    ...
    try:
        ...
        cursor.execute("""UPDATE repairs SET status=%s, estimated_completion=%s, 
                       ...
                       (request.form.get('status'), request.form.get('estimated_completion') or None, 
                        request.form.get('repair_cost') or 0, request.form.get('technician_notes'), repair_id))
        conn.commit()
        cursor.close()
        conn.close()
